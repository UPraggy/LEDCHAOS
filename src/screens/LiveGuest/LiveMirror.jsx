...
/**
 * LiveMirror — o espelho ao vivo do convidado, CEGO ao cano.
 *
 * Pinta a festa a partir de um `link` (o formato que useGuestLink E useDirectGuest
 * produzem, idêntico: status/selfId/players/settings/round/phase/result/final/ping).
 * Não sabe se o estado chegou pelo relay (WS) ou pelo P2P direto — só reflete o que
 * o host, a autoridade, anunciou. Extraído do LiveGuest justamente para o modo
 * direto (zero-servidor) reusar a MESMA tela, sem duplicar a apresentação.
 *
 * @param {object}   props
 * @param {object}   props.link    estado espelhado do host (ver hooks acima)
 * @param {string}   props.code    rótulo da sala no cabeçalho ("A1B2" ou "DIRETO")
 * @param {Function} props.onExit  sair da festa (volta pra Home)
 */
export default function LiveMirror({ link, code, onExit }) {
      ...
      <section className="live__lobby">
        ...
        <p className="live__sub">
          {link.status === LINK.LIVE
            ...
            : 'Abrindo o canal da sala. Isso leva um instante.'}
        </p>
        {players.length ? (
  ...
  return (
    <Screen hue={game?.hue}>
      <header className="live__top">
        <IconButton label="Sair da festa" onClick={onExit}>
          ←
        </IconButton>
